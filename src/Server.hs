...
setupPackets clientsTVar byteLimit seconds = do
	...
	flip evalStateT (PacketState clients 0 initialTime) $ forever $ do
		bs <- lift await
		PacketState cs numBytes time <- get
		diffCs <- liftIO $ readTVarIO clientsTVar
		let	newNum = numBytes + BS.length bs
		let clientChange = not $ length diffCs == length cs
		if clientChange
		then do
...
toManySockets = for cat (\(EncodedPacket socks bs) -> do
	liftIO $ mapM_ (flip PTCP.sendLazy bs) socks)


encodePacket :: Pipe Packet EncodedPacket IO ()
encodePacket = for cat (\(Packet socks wp) -> do
...
addClient tvar = do
	...
	forever $ do
		(cS,_) <- accept s
		atomically $ do
			modifyTVar tvar (\sockets -> sockets ++ [cS])

mainServer :: IO ()
mainServer = do
	clients <- newTVarIO []
	C.forkIO $ addClient clients
...
mainClient = do
	getALReady
	alSource <- source
	PTCP.connect "localhost" "11004" (\(s,a) -> process alSource $ PTCP.fromSocket s 4096)
	where 
	process alSource p = do
		(result,rest) <- PP.runStateT (PA.parse parseWavePacket) p
		runEffect $ getWavePacket result >-> delayPipe >-> alFormatPipe >-> alConsumer alSource
